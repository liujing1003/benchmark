...
def main():
    ...
    if args.tensorboard: configure("runs_WHOI_resnet/%s"%(args.expname))
    ...
    numberofclass = 103  # changed to 89
    if args.dataset == 'cifar100':
        ...
        train_loader = torch.utils.data.DataLoader(
            datasets.ImageFolder('/media/ouc/30bd7817-d3a1-4e83-b7d9-5c0e373ae434/LiuJing/2013', transform=transform_train),
            batch_size=args.batchsize, shuffle=True, **kwargs)
        val_loader = torch.utils.data.DataLoader(
...
def save_checkpoint(state, is_best, filename='checkpoint.pth.tar'):
    ...
    directory = "runs_WHOI_resnet/%s/"%(args.expname)
    if not os.path.exists(directory):
        os.makedirs(directory)
    filename = directory + filename
    torch.save(state, filename)
    if is_best:
        shutil.copyfile(filename, 'runs_WHOI_resnet/%s/'%(args.expname) + 'model_best.pth.tar')
